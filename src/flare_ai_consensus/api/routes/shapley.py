...
class ChatRouter:
    ...
    def __init__(
        ...
        self.logger = logger.bind(router="shapley")
        self._setup_routes()
    ...
    def _setup_routes(self) -> None:
        ...
        @self._router.post("/")
        async def shapley(message: ChatMessage) -> dict[str, str] | None:  # pyright: ignore [reportUnusedFunction]
            """
            Process a chat message through the CL pipeline for shapley calc.
            Returns an aggregated response after a number of iterations.
            """
            try:
                self.logger.debug("Received shapley chat message", message=message.user_message)
